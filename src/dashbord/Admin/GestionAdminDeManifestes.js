...
const GestionAdminDeManifestes = () => {
  ...
  useEffect(() => {
    const fetchManifests = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/manifest/user/${userId}`);
  ...
  const handleDelete = async (manifestId) => {
    try {
      const response = await fetch(`http://localhost:3000/api/manifests/${manifestId}`, {
        method: "DELETE",
      });
  ...
  const handlePrintInvoice = (manifest) => {
    ...
    doc.text(`Nom: ${manifest.expediteur?.firstName} ${manifest.expediteur?.lastName}`, 10, 50);
    ...
    doc.autoTable({
      head: [["CODE", "PRIX", "CLIENT", "ADRESSE", "Designation", "Nb P", "Status"]],
      body: tableData,
      startY: 90,
    });
  ...
  const filteredManifests = manifests.filter((manifest) => {
    ...
    const date = manifest.date ? new Date(manifest.date).toLocaleDateString() : "";
        ...
        <div className="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10 rounded-lg shadow-md">
          <div className="sm:flex items-center justify-between">
            <div className="flex items-center">
              {["All", "Pending", "Shipped", "Delivered", "Retour"].map((tab) => (
                <button
                  key={tab}
                  ...
                  >
                    <td className="pl-5">
                      <p className="text-base font-medium leading-none text-gray-700">
                        {manifest.expediteur?.firstName} {manifest.expediteur?.lastName}
                      </p>
                    </td>
                    <td className="pl-5">
                      <p className="text-base font-medium leading-none text-gray-700">
                    ...
                    <td className="pl-5">
                      <p className="text-base font-medium leading-none text-gray-700">
                        {manifest.date ? new Date(manifest.date).toLocaleDateString() : "N/A"}
                      </p>
                    </td>
                    <td className="pl-5">
                      <p className="text-base font-medium leading-none text-gray-700">
                    ...
                    <td className="pl-5">
                      <div className="flex items-center">
                        ...
                        ></div>
                        <p className="text-sm leading-none text-gray-600">{manifest.status || "N/A"}</p>
                      </div>
                    </td>
                    <td className="pl-5">
...
export default GestionAdminDeManifestes;