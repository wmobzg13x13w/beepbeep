...
export default function Dashboard() {
  ...
  const trueFalseCharts = Object.keys(stats.trueFalseStats).map((key) => ({
    ...
    datasets: [
      {
        data: [
          stats.trueFalseStats[key].trueCount,
          stats.trueFalseStats[key].falseCount,
        ],
      ...
      <main className='flex-1 p-8 overflow-auto'>
        ...
        <section className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8'>
          {[
            ...
            {
              label: "Total des visites VPN",
              value: stats.visits.totalVpnVisits,
            },
            {
              label: "Total des nouveaux membres",
              value: stats.newMembers.totalNewMembers,
            },
            {
              label: "Total des apparitions TV/Radio",
              value: stats.mediaPresence.totalTvRadioAppearances,
            },
          ].map((stat, index) => (
            <div
              key={index}
                ...
                {chart.labels.map((label, i) => (
                  <div key={i} className='flex items-center'>
                    <span
                      ...
                      style={{
                        backgroundColor: chart.datasets[0].backgroundColor[i],
                      }}></span>
        ...
        <section className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
          {[
            {
              label: "Total des partenariats signés",
              value: stats.visits.totalPartnershipsSigned,
            },
            {
              label: "Total des réunions organisées",
              value: stats.activities.totalMeetingsOrganized,
            },
            {
              label: "Total des actions organisées",
              value: stats.activities.totalActionsOrganized,
            },
            {
              label: "Total des assemblées générales",
              value: stats.activities.totalGeneralAssemblies,
            },
            {
              label: "Total des publications sur les réseaux sociaux",
              value: stats.mediaPresence.totalSocialMediaPosts,
            },
            {
              label: "Total des objectifs atteints",
              value: stats.performance.totalObjectivesMet,
            },
          ].map((stat, index) => (
            <div
              key={index}
        ...
        <section className='mt-8'>
          <div className='bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition duration-300'>
            <h3 className='text-lg font-semibold text-gray-800 mb-4'>
              Aperçu des statistiques numériques
            </h3>
